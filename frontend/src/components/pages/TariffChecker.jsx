...
const TariffChecker = () => {
  ...
  // Prefill origin country if supplier is passed from wizard
  useEffect(() => {
    if (location.state && location.state.supplier) {
      setFormData(prev => ({
        ...prev,
        originCountry: location.state.supplier.country
      }))
    }
        ...
        {isWizard && (
          <Box sx={{ maxWidth: 900, mx: 'auto', mt: 2, mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
              <Button variant="outlined" onClick={() => navigate('/suppliers?wizard=1')}>Back</Button>
              <Button variant="contained" onClick={() => navigate('/routes?wizard=1')}>Next</Button>
            </Box>
          </Box>
        )}
